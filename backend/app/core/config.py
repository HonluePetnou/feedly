from pydantic_settings import BaseSettings

class Settings(BaseSettings):
    PROJECT_NAME: str = "Plateforme de Surveillance et d'Aide à la Décision"
    API_V1_STR: str = "/api/v1"
    # Format: postgresql://user:password@postgresserver/db
    SQLALCHEMY_DATABASE_URI: str = "postgresql://postgres:password@localhost:5432/surveillance_db"
    
    SECRET_KEY: str = "09d25e094faa6ca2556c818166b7a9563b93f7099f6f0f4caa6cf63b88e8d3e7" # Change this in production!
    ALGORITHM: str = "HS256"
    ACCESS_TOKEN_EXPIRE_MINUTES: int = 30

    class Config:
        case_sensitive = True

settings = Settings()
